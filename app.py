# app.py — Punto de entrada del observatorio (Streamlit multi-página)
...
st.markdown(
    """
    <style>
        /* Oculta el header de Streamlit y reduce padding superior */
...
with col_titulo:
    st.markdown(
        ...
        <p style='margin:0; color:#95a5a6; font-size:0.95rem;'>
            Valle de Comayagua, Honduras — Estimación temprana de rendimiento mediante imágenes Sentinel-2
        </p>
        """,
        unsafe_allow_html=True,
    )
...
with col_izq:
    ...
    st.markdown(
        """
        Este observatorio es la capa de visualización del pipeline de estimación temprana
        de rendimiento de maíz desarrollado como parte de la tesis:
    ...
    st.info(
        "📌 Selecciona una vista en el menú lateral para explorar el observatorio.",
        icon=None,
    )
...
with c1:
    st.markdown(
        f"""
        <div style='{_nav_style} border-top:3px solid #2ecc71;'>
            <div style='font-size:1.6rem;'>🗺️</div>
            <div style='font-weight:700; margin:.4rem 0 .25rem;'>Parcelas</div>
            <div style='color:#95a5a6; font-size:.85rem;'>
                Mapa interactivo de parcelas coloreadas por cultivo o rendimiento estimado.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
with c2:
    st.markdown(
        f"""
        <div style='{_nav_style} border-top:3px solid #3498db;'>
            <div style='font-size:1.6rem;'>📈</div>
            <div style='font-weight:700; margin:.4rem 0 .25rem;'>Series Temporales</div>
            <div style='color:#95a5a6; font-size:.85rem;'>
                Curvas EVI, LSWI y GPP diario por parcela con marcadores SOS y POS.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
with c3:
    st.markdown(
        f"""
        <div style='{_nav_style} border-top:3px solid #e67e22;'>
            <div style='font-size:1.6rem;'>⚖️</div>
            <div style='font-weight:700; margin:.4rem 0 .25rem;'>Estimación</div>
            <div style='color:#95a5a6; font-size:.85rem;'>
                qq/ha estimado vs referencia SAG/CAN por ventana T1, T2 y T3.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
...
with c4:
    st.markdown(
        f"""
        <div style='{_nav_style} border-top:3px solid #9b59b6;'>
            <div style='font-size:1.6rem;'>📊</div>
            <div style='font-weight:700; margin:.4rem 0 .25rem;'>Resumen Valle</div>
            <div style='color:#95a5a6; font-size:.85rem;'>
                Producción total agregada en quintales y métricas de validación.
            </div>
        </div>
        """,
        unsafe_allow_html=True,
